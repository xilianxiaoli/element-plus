...
const FormGridInner = observer(
  defineComponent({
    ...
    setup(props, { slots }) {
      ...
      const w = ref({ age: 100 })
      setTimeout(() => {
        w.value.age = w.value.age + 1
      })

      const style = computed(() => {
        return {
          width: w.value.age + 'px',
        }
      })

      return () => {

        // TODO 无法获取 gridInstance.value.templateColumns ，只有强制触发 computed 才能拿到最新数据
        console.log('render style:', style.value);
        ...
        return h(
          'div',
          {
            ...
            ref: 'root',
            pp: gridInstance.value.templateColumns
          },
          slots
        )
      }
