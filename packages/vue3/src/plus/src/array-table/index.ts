...
import type {
  ElTable as TableProps,
  ElTableColumn as ElColumnProps,
  ElPagination as PaginationProps,
} from 'element-plus'
...
type IArrayTableProps = {
  pagination?: typeof PaginationProps | boolean
} & typeof TableProps

type IArrayTablePaginationProps = {
  dataSource?: any[]
...
interface ObservableColumnSource {
  ...
  columnProps: typeof ElColumnProps & { title: string; asterisk: boolean }
  schema: Schema
...
type ColumnProps = typeof ElColumnProps & {
  key: string | number
  asterisk: boolean
  render?: (
            ...
            default: () => {
              return props.options?.map(({ label, value }) => {
                const hasError = errors.some(({ address }) => {
                  ...
                  const startIndex = (value - 1) * (props.pageSize as number)
                  const endIndex = value * (props.pageSize as number)
      ...
      return () => {
        ...
        const renderColumns = (startIndex?: Ref<number>) => {
          return columns.map(({ key, render, asterisk, ...props }) => {
            ...
            if (asterisk) {
              children.header = ({ column }: { column: typeof ElColumnProps }) =>
                h(
                  'span',
                  {},
                  {
